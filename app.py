...
def firecrawl_search(query: str, limit: int = 10, country: str = 'US', location: str = '') -> Dict[str, Any]:
...
def map_modifiers_to_intents(q: str, mods: Dict[str, List[str]]) -> Dict[str,float]:
    score = {i:0.0 for i in INTENTS}
...
def analyze_top_pages(items: List[Dict[str,Any]], max_pages: int) -> Tuple[Dict[str,float], List[Dict[str,Any]]]:
    score = {i:0.0 for i in INTENTS}
    notes = []
...
def label_from_scores(scores: Dict[str,float]) -> Tuple[str,str,float,str]:
    s = sum(scores.values()) or 1e-6
    normed = {k:v/s for k,v in scores.items()}
    primary = max(normed, key=normed.get)
    secondary = sorted([k for k in normed if k!=primary], key=lambda x: normed[x], reverse=True)[0]
    mixed = [k for k,v in normed.items() if v >= 0.30]
    branching = 'Mixed Intent' if len(mixed) >= 2 else 'Clear'
    top1, top2 = normed[primary], normed[secondary]
    confidence = max(0.0, (top1-top2))*100.0
...
def run_for_keyword(q: str) -> Dict[str, Any]:
    ...
    mods = {
        'informational':[m.strip() for m in informational_mods.split(',') if m.strip()],
    ...
    if df is None or df.empty:
        ...
        for _, r in fdf.sort_values(by=['confidence_pct','primary_intent'], ascending=[False, True]).iterrows():
            ...
            <div class="card">
              ...
              <div class="kv"><b>Secondary:</b> {r["secondary_intent"]} • <b>Confidence:</b> {r["confidence_pct"]:.1f}% • <b>Status:</b> {r["branching"]}</div>
              <div class="smallmuted">{", ".join(urls[:3]) if urls else "No URLs captured"}</div>
            </div>
            ''', unsafe_allow_html=True)
        ...
        st.dataframe(
            ...
            column_config={
                'confidence_pct': st.column_config.ProgressColumn('Confidence %', max_value=100, format='%.1f%%'),
...
with tab_overview:
    ...
    if df is None or df.empty:
        ...
        fig2 = px.box(df, x='primary_intent', y='confidence_pct', color='primary_intent',
