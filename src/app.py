...
def hero_banner():
    st.markdown(
        f'<div style="text-align:center;padding:18px 8px;border-top:1px solid #e6e6e6;"><h2 style="margin:6px 0 10px 0;">Search Intent Analyzer v{__version__}</h2><p style="margin:6px 0 6px 0;">Built by <strong>{AUTHOR}</strong></p><p style="margin:0;">Powered by Firecrawl + Hybrid Classifier</p></div>',
...
def app_footer():
    st.markdown(
        f'<hr style="margin-top:36px;"/><div style="text-align:center;font-size:13px;color:#666;padding-bottom:12px;"><div><strong>Search Intent Analyzer</strong> • v{__version__}</div><div>© {dt.datetime.now().year} {AUTHOR} — MIT License</div></div>',
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    st.subheader("Rules")
    informational_mods = st.text_area(
        "Informational",
        "how,what,why,who,guide,tutorial,learn,meaning,definition,ideas,examples,steps",
    )
    transactional_mods = st.text_area(
        "Transactional", "buy,price,deal,discount,coupon,book,order,subscribe,download"
    )
    navigational_mods = st.text_area(
        "Navigational", "brand,login,official,homepage,near me,locations,contact"
    )
    commercial_mods = st.text_area(
...
def contains_any(s, words):
    s_low = s.lower()
    return any(w.strip().lower() in s_low for w in words if w.strip())
...
def map_features_to_intents_from_fc(items, query):
    ...
    joined = " \n ".join(texts).lower()
    if any(
        k in joined for k in ["faq", "people also ask", "how to", "guide", "tutorial"]
    ):
        score["Informational"] += 0.6
    if any(k in joined for k in ["buy", "price", "add to cart", "checkout", "shop"]):
        score["Transactional"] += 0.6
    if any(k in joined for k in ["official site", "login", "contact us"]):
        score["Navigational"] += 0.4
    if any(k in joined for k in ["review", "best", "top", " vs ", "comparison"]):
        score["Commercial Investigation"] += 0.6
    if any(v in joined for v in INTEGRATION_VERBS):
        score["Informational"] += 0.8
    if detect_brand_pairs(texts) >= 2:
        score["Informational"] += 0.6
    return score
...
def map_modifiers_to_intents(q, mods):
    score = {i: 0.0 for i in INTENTS}
    if contains_any(q, mods["informational"]):
        score["Informational"] += 1
    if contains_any(q, mods["transactional"]):
        score["Transactional"] += 1
    if contains_any(q, mods["navigational"]):
        score["Navigational"] += 1
    if contains_any(q, mods["commercial"]):
        score["Commercial Investigation"] += 1
    if contains_any(q, INTEGRATION_VERBS):
        score["Informational"] += 0.8
    if detect_brand_pairs([q]) >= 2:
        score["Informational"] += 0.6
    return score
...
def analyze_top_pages(items, max_pages):
    ...
    for it in items[:max_pages]:
        ...
        ctas = [
            ...
            if t in markdown or t in html
        ]
        schema = []
        if "faq" in markdown or "faqpage" in html:
            schema.append("FAQ")
        if "howto" in html or "how-to" in html:
            schema.append("HowTo")
        if "product" in html and ("price" in html or "sku" in html):
            schema.append("Product")
        if "review" in markdown or "aggregaterating" in html:
            schema.append("Review")
        if ctas or "Product" in schema:
            score["Transactional"] += 1
        if "FAQ" in schema or "HowTo" in schema:
            score["Informational"] += 1
        if "Review" in schema:
            score["Commercial Investigation"] += 1
        if "Product" in schema and ("FAQ" in schema or "HowTo" in schema):
            score["Informational"] += 0.7
            score["Transactional"] = max(0.0, score["Transactional"] - 0.3)
        if any(v in markdown for v in INTEGRATION_VERBS):
            score["Informational"] += 0.5
...
def combine_scores(
    s_serp, s_rules, s_pages, w_serp, w_rules, w_pages, s_clf=None, clf_weight=0
):
    ...
    for i in INTENTS:
        ...
        if s_clf:
            base = (1 - clf_weight / 100.0) * base + (clf_weight / 100.0) * s_clf.get(
                i, 0.0
            )
        combined[i] = base
    return combined
...
def label_from_scores(scores):
    ...
    secondary = sorted(
        [k for k in probs if k != primary], key=lambda x: probs[x], reverse=True
    )[0]
    top1 = probs[primary]
    top2 = probs[secondary]
    margin = (top1 - top2) / (top1 + top2 + eps)
    ent = -sum(p * math.log(p + eps) for p in probs.values()) / math.log(len(probs))
    confidence = max(0.0, (1 - ent)) * margin * 100.0
    branching = "Mixed Intent" if ent > 0.85 else "Clear"
...
with tab_input:
    ...
    kws_text = st.text_area(
        "One keyword per line", height=160, placeholder="e.g. alexa homekit"
    )
...
def run_for_keyword(q):
    ...
    mods = {
        "informational": [
            m.strip() for m in informational_mods.split(",") if m.strip()
        ],
        "transactional": [
            m.strip() for m in transactional_mods.split(",") if m.strip()
        ],
...
if run_btn:
    ...
    if not FIRECRAWL_KEY:
        st.error("Please provide FIRECRAWL_API_KEY in the sidebar or environment.")
    elif not kw_list:
...
with tab_results:
    ...
    if df is None or df.empty:
        ...
        with c1:
            st.metric("Keywords", len(fdf))
        with c2:
            st.metric(
                "Avg confidence",
                f"{fdf['confidence_pct'].mean():.1f}%" if len(fdf) else "—",
            )
        with c3:
            st.metric(
                "Mixed intents",
                int((fdf["branching"] == "Mixed Intent").sum()) if len(fdf) else 0,
            )
        with c4:
            st.metric(
                "Clear intents",
                int((fdf["branching"] == "Clear").sum()) if len(fdf) else 0,
            )
        st.divider()
        ...
        for _, r in fdf.sort_values(
            ...
            urls = json.loads(r["top_urls"] or "[]")
            badge_html = badge(r["primary_intent"], color)
            st.markdown(
                f'<div class="card"><h4>{r["keyword"]} {badge_html}</h4><div class="kv"><b>Secondary:</b> {r["secondary_intent"]} • <b>Confidence:</b> {r["confidence_pct"]:.1f}% • <b>Status:</b> {r["branching"]}</div><div class="smallmuted">{", ".join(urls[:3]) if urls else "No URLs captured"}</div></div>',
                unsafe_allow_html=True,
            )
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "top_urls": st.column_config.TextColumn("Top URLs (JSON)"),
            },
        )
        st.download_button(
            "Download filtered CSV",
            fdf.to_csv(index=False).encode("utf-8"),
            "intent_report_filtered.csv",
            "text/csv",
        )
...
with tab_overview:
    ...
    if df is None or df.empty:
        ...
        counts = df["primary_intent"].value_counts().reset_index()
        counts.columns = ["intent", "count"]
        fig = px.pie(counts, names="intent", values="count", hole=0.45)
